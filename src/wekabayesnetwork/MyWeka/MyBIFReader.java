 ...
 * stored in XML BIF 0.3 format.<br/> <br/> For more details on XML BIF
 * see:<br/> <br/> Fabio Cozman, Marek Druzdzel, Daniel Garcia (1998). XML BIF
 * version 0.3. URL
        ...
        implements TechnicalInformationHandler
{
     ...
     * @return the MyBIFReader
     * @throws Exception if processing fails
     */
    public MyBIFReader processFile(String sFile) throws Exception
    {
        m_sFile = sFile;
    ...
    public MyBIFReader processString(String sStr) throws Exception
    {
        DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
        factory.setValidating(true);
    ...
    /**
     * buildStructure parses the BIF document in the DOM tree contained in the
     * doc parameter and specifies the the network structure and probability
     * tables. It assumes that buildInstances has been called before
     *
    ...
    void buildStructure(Document doc) throws Exception
    {
        ...
        for (int iNode = 0; iNode < m_Instances.numAttributes(); iNode++)
        {
            ...
            for (int iParent = 0; iParent < nodelist.size(); iParent++)
            {
                ...
                m_ParentSets[iNode].addParent(nParent,
                                              m_Instances);
            }
            ...
            {
                m_Distributions[iNode][i] = new DiscreteEstimatorBayes(nValues,
                                                                       0.0f);
            }
    ...
    /**
     * Returns all TEXT children of the given node in one string. Between the
     ...
     */
    void buildInstances(Document doc,
                        String sName) throws Exception
    {
        ...
        for (int iNode = 0; iNode < nodelist.getLength(); iNode++)
        {
            ...
            weka.core.Attribute att = new weka.core.Attribute(sNodeName,
                                                              nomStrings);
        ...
        m_Instances = new Instances(sName,
                                    attInfo,
                                    100);
...
//	} // selectNodeList
    ...
    FastVector getParentNodes(Node definition) throws Exception
    {
        ...
        FastVector nodelist = selectElements(definition,
                                             "GIVEN");
        return nodelist;
    } // getParentNodes
    ...
    FastVector selectOutCome(Node item) throws Exception
    {
        ...
        FastVector nodelist = selectElements(item,
                                             "OUTCOME");
        return nodelist;
    } // selectOutCome
            ...
            for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++)
            {
                for (int iParent = 0; iParent < m_ParentSets[iAttribute].getNrOfParents(); iParent++)
                {
                    ...
                    if (!other.getParentSet(m_order[iAttribute]).contains(
                            m_order[nParent]) && !other.getParentSet(
                            m_order[nParent]).contains(m_order[iAttribute]))
                    {
                        nMissing++;
                    }
                }
    ...
    public int extraArcs(BayesNet other)
    {
        try
        {
            ...
            for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++)
            {
                for (int iParent = 0; iParent < other.getParentSet(
                        m_order[iAttribute]).getNrOfParents(); iParent++)
                {
                    int nParent = m_order[other.getParentSet(m_order[iAttribute]).getParent(
                            iParent)];
     ...
     * represented by this network and that of another, that is, \sum_{x\in X}
     * P(x)log P(x)/Q(x) where X is the set of values the nodes in the network
     * can take, P(x) the probability of this network for configuration x Q(x)
     * the probability of the other network for configuration x
     *
     * @param other network to compare with
            ...
            while (i < nNodes)
            {
                ...
                if (i < nNodes)
                {
                    ...
                    for (int iNode = 0; iNode < nNodes; iNode++)
                    {
                        ...
                        P = P * m_Distributions[iNode][iCPT].getProbability(
                                x[iNode]);
                    }
                    ...
                    for (int iNode = 0; iNode < nNodes; iNode++)
                    {
                        int iCPT = 0;
                        for (int iParent = 0; iParent < other.getParentSet(
                                m_order[iNode]).getNrOfParents(); iParent++)
                        {
                            int nParent = m_order[other.getParentSet(
                                    m_order[iNode]).getParent(iParent)];
                            iCPT = iCPT * nCard[nParent] + x[nParent];
                        }
                        Q = Q * other.m_Distributions[m_order[iNode]][iCPT].getProbability(
                                x[iNode]);
                    }
    ...
    public static void main(String[] args)
    {
        try
        {
            MyBIFReader br = new MyBIFReader();
