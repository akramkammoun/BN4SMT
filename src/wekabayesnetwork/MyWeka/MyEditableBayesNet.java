 ...
 */
package wekabayesnetwork.MyWeka;
 ...
 * algorithms and quality measures.<br/> Base class for a Bayes Network
 * classifier. Provides datastructures (network structure, conditional
 * probability distributions, etc.) and facilities common to Bayes Network
 * learning algorithms like K2 and B.<br/> <br/> For more information see:<br/>
 * <br/> http://www.cs.waikato.ac.nz/~remco/weka.pdf
 * <p/>
 * <!-- globalinfo-end -->
 *
 ...
 */
public class MyEditableBayesNet extends BayesNet
{
     ...
     */
    public FastVector m_nPositionX;
    public FastVector m_nPositionY;
    /**
...
//		m_fMarginP = new FastVector(0);
    ...
    /**
     * constructor, creates empty network with nodes based on the attributes in
     * a data set
     */
    public MyEditableBayesNet(Instances instances)
    {
        try
        {
        ...
        for (int iNode = 0; iNode < nNodes; iNode++)
        {
            ...
            m_Distributions[iNode][0] = new DiscreteEstimatorBayes(getCardinality(
                    iNode),
                                                                   0.5);
        }
...
//		}
     ...
     * encapsulated in a MyBIFReader
     */
    public MyEditableBayesNet(MyBIFReader other)
    {
     ...
     *
     * @param bSetInstances flag indicating whether to initialize instances or
     * not
     */
    public MyEditableBayesNet(boolean bSetInstances)
    {
        super();
        m_nEvidence = new FastVector(0);
        m_fMarginP = new FastVector(0);
        ...
        if (bSetInstances)
        {
            m_Instances = new Instances("New Network",
                                        new FastVector(0),
                                        0);
        }
    } // c'tor
     ...
     * the data set must be put if correct order first and possibly
     * discretized/missing values filled in before proceeding to CPT learning.
     *
     * @param instances data set to learn from
     * @exception Exception when data sets are not compatible, e.g., a variable
     * is missing or a variable has different nr of values.
     */
    public void setData(Instances instances) throws Exception
    {
        ...
        for (int iNode = 0; iNode < getNrOfNodes(); iNode++)
        {
            ...
            int nNode = 0;
            while (nNode < getNrOfNodes() && !sName.equals(instances.attribute(
                    nNode).name()))
            {
                nNode++;
            }
            ...
            {
                throw new Exception(
                        "Cannot find node named [[[" + sName + "]]] in the data");
            }
            order[iNode] = nNode;
        }
        ...
        else
        {
            ...
            {
                newInstances.add(
                        normalizeInstance(instances.instance(iInstance)));
            }
        }
        //sanity check
        for (int iNode = 0; iNode < getNrOfNodes(); iNode++)
        {
            if (newInstances.attribute(iNode).numValues() != getCardinality(
                    iNode))
            {
                throw new Exception("Number of values of node [[[" + getNodeName(
                        iNode) + "]]] differs in (discretized) dataset.");
            }
        }
     ...
     * @throws Exception
     */
    public void addNode(String sName,
                        int nCardinality) throws Exception
    {
        addNode(sName,
                nCardinality,
                100 + getNrOfNodes() * 10,
                        ...
                        int nPosY) throws Exception
    {
        ...
        Attribute att = new Attribute(sName,
                                      values);
        m_Instances.insertAttributeAt(att,
        ...
        distributions[nAtts - 1][0] = new DiscreteEstimatorBayes(nCardinality,
                                                                 0.5);
        ...
        for (int iValue = 0; iValue < nCardinality; iValue++)
        {
            fMarginP[iValue] = (float) (1.0 / nCardinality);
        }
        m_fMarginP.addElement(fMarginP);
        // update undo stack
        if (m_bNeedsUndoAction)
        {
            addUndoAction(new AddNodeAction(sName,
                                            nCardinality,
                                            nPosX,
                                            nPosY));
        }
     ...
     *
     * @param sName name of the node. If the name does not exists an exception
     * is thrown
     * @throws Exception
     */
    ...
    public void deleteNode(int nTargetNode) throws Exception
    {
        ...
        for (int iParentSet = 0; iParentSet < nAtts; iParentSet++)
        {
            ...
            parentset.deleteParent(nTargetNode,
                                   m_Instances);
        ...
        for (int i = 0; i < nodes.size(); i++)
        {
            for (int j = i + 1; j < nodes.size(); j++)
            {
                if ((Integer) nodes.elementAt(i) > (Integer) nodes.elementAt(j))
                {
                    ...
                    nodes.setElementAt(nodes.elementAt(j),
                                       i);
                    nodes.setElementAt(h,
                                       j);
                }
            }
        }
     ...
     */
    FastVector selectElements(Node item,
                              String sElement) throws Exception
    {
    ...
    /**
     * XML helper function that returns DEFINITION element from a XMLBIF
     * document for a node with a given name.
     *
     * @param doc XMLBIF document
     * @param sName name of the node to get the definition for
     */
    Element getDefinition(Document doc,
                          String sName) throws Exception
    {
        ...
        for (int iNode = 0; iNode < nodelist.getLength(); iNode++)
        {
            ...
            FastVector list = selectElements(node,
                                             "FOR");
            if (list.size() > 0)
            {
     ...
     */
    void paste(String sXML,
               int mode) throws Exception
    {
        DocumentBuilderFactory factory = DocumentBuilderFactory.newInstance();
        factory.setValidating(true);
        Document doc = factory.newDocumentBuilder().parse(new org.xml.sax.InputSource(new StringReader(
                sXML)));
        doc.normalize();
        ...
        for (int iNode = 0; iNode < nodelist.getLength(); iNode++)
        {
            ...
            valueslist = selectElements(nodelist.item(iNode),
                                        "OUTCOME");
            ...
            nodelist2 = selectElements(nodelist.item(iNode),
                                       "NAME");
            if (nodelist2.size() == 0)
            {
            ...
            Attribute att = new Attribute(sNodeName,
                                          nomStrings);
            instances.insertAttributeAt(att,
            ...
            for (int iValue = 0; iValue < nValues; iValue++)
            {
                ...
                if (sValue.startsWith("position"))
                {
                    ...
                    String sX = sValue.substring(i0 + 1,
                                                 i1).trim();
                    String sY = sValue.substring(i1 + 1,
                                                 i2).trim();
                    try
                    {
                    ...
                    {
                        System.err.println(
                                "Wrong number format in position :(" + sX + "," + sY + ")");
                    }
                }
            }
            if (mode == EXECUTE)
        ...
        for (int iNode = 0; iNode < sBaseNames.size(); iNode++)
        {
            ...
            Element definition = getDefinition(doc,
                                               sName);
            parentsets[nBase + iNode] = new ParentSet();

            // get the parents for this node
            // resolve structure
            nodelist2 = selectElements(definition,
                                       "GIVEN");
            for (int iParent = 0; iParent < nodelist2.size(); iParent++)
            {
                ...
                parentsets[nBase + iNode].addParent(nParent,
                                                    instances);
            }
            ...
            {
                distributions[nBase + iNode][i] = new DiscreteEstimatorBayes(
                        nValues,
                                                                             0.0f);
            }

            String sTable = getContent((Element) selectElements(definition,
                                                                "TABLE").elementAt(
                    0));
            ...
            for (int i = 0; i < nCardinality; i++)
            {
                ...
                for (int iValue = 0; iValue < nValues; iValue++)
                {
                    ...
                    d.addValue(iValue,
                               new Double(sWeight).doubleValue());
                }
            }
            if (mode == EXECUTE)
            {
                m_nEvidence.insertElementAt(-1,
                                            nBase + iNode);
                m_fMarginP.insertElementAt(new float[getCardinality(
                        nBase + iNode)],
                                           nBase + iNode);
            }
        }
        ...
        {
            addUndoAction(new PasteAction(sXML,
                                          nBase));
        }
    } // paste
     ...
     */
    public void addArc(String sParent,
                       String sChild) throws Exception
    {
                       ...
                       int nChild) throws Exception
    {
        ...
        if (m_bNeedsUndoAction)
        {
            addUndoAction(new AddArcAction(nParent,
                                           nChild));
        }
        int nOldCard = m_ParentSets[nChild].getCardinalityOfParents();
        // update parentsets
        m_ParentSets[nChild].addParent(nParent,
                                       m_Instances);
        // update distributions
        ...
        {
            ds[iParent] = Estimator.clone(
                    m_Distributions[nChild][iParent % nOldCard]);
        }
     ...
     */
    public void addArc(String sParent,
                       FastVector nodes) throws Exception
    {
        int nParent = getNode(sParent);
        // update undo stack
        if (m_bNeedsUndoAction)
     ...
     * @throws Exception if parent or child cannot be found in network
     */
    public void deleteArc(String sParent,
                          String sChild) throws Exception
    {
        int nParent = getNode(sParent);
        int nChild = getNode(sChild);
                          ...
                          int nChild) throws Exception
    {
        ...
        if (m_bNeedsUndoAction)
        {
            addUndoAction(new DeleteArcAction(nParent,
                                              nChild));
        }
        // update distributions
        // condense distribution, use values for targetnode = 0
        ...
        // update parentsets
        m_ParentSets[nChild].deleteParent(nParent,
                                          m_Instances);
    } // deleteArc
     ...
     * @throws Exception if parent or child cannot be found in network
     */
    public void setDistribution(String sName,
                                float[][] P) throws Exception
    {
        int nTargetNode = getNode(sName);
        setDistribution(nTargetNode,
                        P);
    } // setDistribution
                                ...
                                float[][] P) throws Exception
    {
        ...
        if (m_bNeedsUndoAction)
        {
            addUndoAction(new SetDistributionAction(nTargetNode,
                                                    P));
        }
    ...
    public float[][] getDistribution(int nTargetNode)
    {
        ...
        for (int iParent = 0; iParent < nParentCard; iParent++)
        {
            ...
            {
                P[iParent][iValue] = (float) m_Distributions[nTargetNode][iParent].getProbability(
                        iValue);
            }
        }
        return P;
     ...
     */
    public String getValueName(int nTargetNode,
                               int iValue)
    {
                            ...
                            String sName)
    {
        ...
        if (m_bNeedsUndoAction)
        {
            addUndoAction(new RenameAction(nTargetNode,
                                           getNodeName(nTargetNode),
                                           sName));
        }
                                ...
                                String sNewValue)
    {
        ...
        if (m_bNeedsUndoAction)
        {
            addUndoAction(new RenameValueAction(nTargetNode,
                                                sValue,
                                                sNewValue));
        }
        Attribute att = m_Instances.attribute(nTargetNode);
     ...
     * probability to the new value. Child nodes duplicate CPT conditioned on
     * the new value.
     *
                             ...
                             String sNewValue)
    {
        ...
        if (m_bNeedsUndoAction)
        {
            addUndoAction(new AddValueAction(nTargetNode,
                                             sNewValue));
        }
        Attribute att = m_Instances.attribute(nTargetNode);
        int nCardinality = att.numValues();
        ...
        for (int iParent = 0; iParent < distributions.length; iParent++)
        {
            ...
            {
                distribution.addValue(iValue,
                                      distributions[iParent].getProbability(
                        iValue));
            }
            distributions[iParent] = distribution;
        }
        ...
        for (int iNode = 0; iNode < getNrOfNodes(); iNode++)
        {
            if (m_ParentSets[iNode].contains(nTargetNode))
            {
                ...
                int nParentCard = parentSet.getFreshCardinalityOfParents(
                        m_Instances);
                ...
                for (int iPos = 0; iPos < nParentCard; iPos++)
                {
                    ...
                    {
                        distribution.addValue(iValue,
                                              distributions[iOldPos].getProbability(
                                iValue));
                    }
                    ...
                    int i = 0;
                    values2[i]++;
                    while (i < nParents && values2[i] == getCardinality(parentSet.getParent(
                            i)))
                    {
                        values2[i] = 0;
                        i++;
     ...
     * proportional to existing distribution (or made uniform if zero
     * probability is assigned to remainder of values). .* Child nodes delete
     * CPTs conditioned on the new value.
     *
                             ...
                             String sValue) throws Exception
    {
        ...
        if (m_bNeedsUndoAction)
        {
            addUndoAction(new DelValueAction(nTargetNode,
                                             sValue));
        }
        Attribute att = m_Instances.attribute(nTargetNode);
        int nCardinality = att.numValues();
        ...
        if (nValue < 0)
        {
            ...
            throw new Exception(
                    "Node " + nTargetNode + " does not have value (" + sValue + ")");
        }
        replaceAtt(nTargetNode,
                   att.name(),
                   values);
        ...
        for (int iNode = 0; iNode < getNrOfNodes(); iNode++)
        {
            if (m_ParentSets[iNode].contains(nTargetNode))
            {
                ...
                {
                    nCards[iParent] = getCardinality(
                            parentSet.getParent(iParent));
                }
                nCards[iTargetNode]++;
                for (int iPos = 0; iPos < nParentCard; iPos++)
                {
        ...
        {
            setEvidence(nTargetNode,
                        getEvidence(nTargetNode) - 1);
        }
    } // delNodeValue
                            ...
                            int nY)
    {
        ...
        if (m_bNeedsUndoAction)
        {
            ...
            if (!isUpdate)
            {
                addUndoAction(new SetPositionAction(iNode,
                                                    nX,
                                                    nY));
            }
        }
        m_nPositionX.setElementAt(nX,
                                  iNode);
        m_nPositionY.setElementAt(nY,
                                  iNode);
    } // setPosition
                            ...
                            FastVector nodes)
    {
        ...
        if (m_bNeedsUndoAction)
        {
            ...
            try
            {
                ...
                if (m_undoStack.size() > 0)
                {
                    ...
                    {
                        if ((Integer) posAction.m_nodes.elementAt(iNode) != (Integer) nodes.elementAt(
                                iNode))
                        {
                            isUpdate = false;
                        }
            ...
            if (!isUpdate)
            {
                addUndoAction(new SetGroupPositionAction(nodes,
                                                         dX,
                                                         dY));
            }
        }
        for (int iNode = 0; iNode < nodes.size(); iNode++)
        {
            ...
            m_nPositionX.setElementAt(getPositionX(nNode) + dX,
                                      nNode);
            m_nPositionY.setElementAt(getPositionY(nNode) + dY,
                                      nNode);
        }
    } // setPosition
                            ...
                            FastVector nPosY)
    {
        if (m_bNeedsUndoAction)
        {
            addUndoAction(new LayoutGraphAction(nPosX,
                                                nPosY));
        }
        m_nPositionX = nPosX;
        m_nPositionY = nPosY;
    ...
    public void alignLeft(FastVector nodes)
    {
        ...
        {
            int nNode = (Integer) nodes.elementAt(iNode);
            m_nPositionX.setElementAt(nMinX,
                                      nNode);
        }
    } // alignLeft
    ...
    public void alignRight(FastVector nodes)
    {
        ...
        {
            int nNode = (Integer) nodes.elementAt(iNode);
            m_nPositionX.setElementAt(nMaxX,
                                      nNode);
        }
    } // alignRight
    ...
    public void alignTop(FastVector nodes)
    {
        ...
        {
            int nNode = (Integer) nodes.elementAt(iNode);
            m_nPositionY.setElementAt(nMinY,
                                      nNode);
        }
    } // alignTop
    ...
    public void alignBottom(FastVector nodes)
    {
        ...
        {
            int nNode = (Integer) nodes.elementAt(iNode);
            m_nPositionY.setElementAt(nMaxY,
                                      nNode);
        }
    } // alignBottom
    ...
    public void centerHorizontal(FastVector nodes)
    {
        ...
        {
            int nNode = (Integer) nodes.elementAt(iNode);
            m_nPositionY.setElementAt((nMinY + nMaxY) / 2,
                                      nNode);
        }
    } // centerHorizontal
    ...
    public void centerVertical(FastVector nodes)
    {
        ...
        {
            int nNode = (Integer) nodes.elementAt(iNode);
            m_nPositionX.setElementAt((nMinX + nMaxX) / 2,
                                      nNode);
        }
    } // centerVertical

    /**
     * space out set of nodes evenly between left and right most node in the
     * list
     *
    ...
    /**
     * space out set of nodes evenly between top and bottom most node in the
     * list
     *
     * @param nodes list of indexes of nodes to space out
     */
     ...
     */
    void replaceAtt(int nTargetNode,
                    String sName,
                    FastVector values)
    {
        Attribute newAtt = new Attribute(sName,
                                         values);
        if (m_Instances.classIndex() == nTargetNode)
        {
            m_Instances.setClassIndex(-1);
            m_Instances.insertAttributeAt(newAtt,
                                          nTargetNode);
    ...
    {
        return (float[]) m_fMarginP.elementAt(iNode);
    }
	...
	public void setMargin(int iNode,
                          float[] fMarginP)
    {
        m_fMarginP.setElementAt(fMarginP,
                                iNode);
    }
    ...
    public void setEvidence(int iNode,
                            int iValue)
    {
        m_nEvidence.setElementAt(iValue,
                                 iNode);
    }
    ...
    public String toXMLBIF03()
    {
        ...
        text.append("<NETWORK>\n");
        text.append(
                "<NAME>" + XMLNormalize(m_Instances.relationName()) + "</NAME>\n");
        for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++)
        {
            text.append("<VARIABLE TYPE=\"nature\">\n");
            text.append("<NAME>" + XMLNormalize(
                    m_Instances.attribute(iAttribute).name()) + "</NAME>\n");
            for (int iValue = 0; iValue < m_Instances.attribute(iAttribute).numValues(); iValue++)
            {
                text.append("<OUTCOME>" + XMLNormalize(m_Instances.attribute(
                        iAttribute).value(iValue))
                            + "</OUTCOME>\n");
            }
            text.append("<PROPERTY>position = (" + getPositionX(iAttribute) + "," + getPositionY(
                    iAttribute)
                        + ")</PROPERTY>\n");
        ...
        for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++)
        {
            ...
            text.append("<FOR>" + XMLNormalize(
                    m_Instances.attribute(iAttribute).name()) + "</FOR>\n");
            for (int iParent = 0; iParent < m_ParentSets[iAttribute].getNrOfParents(); iParent++)
            {
                text.append("<GIVEN>"
                            + XMLNormalize(m_Instances.attribute(m_ParentSets[iAttribute].getParent(
                        iParent)).name())
                            + "</GIVEN>\n");
            }
            text.append("<TABLE>\n");
            for (int iParent = 0; iParent < m_ParentSets[iAttribute].getCardinalityOfParents(); iParent++)
            {
                for (int iValue = 0; iValue < m_Instances.attribute(iAttribute).numValues(); iValue++)
                {
                    text.append(m_Distributions[iAttribute][iParent].getProbability(
                            iValue));
                    text.append(' ');
                }
                text.append('\n');
    ...
    public String toXMLBIF03(FastVector nodes)
    {
        ...
        for (int iNode = 0; iNode < nodes.size(); iNode++)
        {
            ...
            text.append("<VARIABLE TYPE=\"nature\">\n");
            text.append("<NAME>" + XMLNormalize(
                    m_Instances.attribute(nNode).name()) + "</NAME>\n");
            for (int iValue = 0; iValue < m_Instances.attribute(nNode).numValues(); iValue++)
            {
                text.append("<OUTCOME>" + XMLNormalize(m_Instances.attribute(
                        nNode).value(iValue)) + "</OUTCOME>\n");
            }
            text.append("<PROPERTY>position = (" + getPositionX(nNode) + "," + getPositionY(
                    nNode) + ")</PROPERTY>\n");
            text.append("</VARIABLE>\n");
        }
        ...
        for (int iNode = 0; iNode < nodes.size(); iNode++)
        {
            ...
            text.append("<DEFINITION>\n");
            text.append("<FOR>" + XMLNormalize(
                    m_Instances.attribute(nNode).name()) + "</FOR>\n");
            for (int iParent = 0; iParent < m_ParentSets[nNode].getNrOfParents(); iParent++)
            {
                text.append("<GIVEN>"
                            + XMLNormalize(m_Instances.attribute(m_ParentSets[nNode].getParent(
                        iParent)).name())
                            + "</GIVEN>\n");
            }
            text.append("<TABLE>\n");
            for (int iParent = 0; iParent < m_ParentSets[nNode].getCardinalityOfParents(); iParent++)
            {
                for (int iValue = 0; iValue < m_Instances.attribute(nNode).numValues(); iValue++)
                {
                    text.append(m_Distributions[nNode][iParent].getProbability(
                            iValue));
                    text.append(' ');
                }
                text.append('\n');
    ...
    /**
     * return true when current state differs from the state the network was
     * last saved
     */
    ...
    public String undo()
    {
        ...
        UndoAction undoAction = (UndoAction) m_undoStack.elementAt(
    ...
    class DeleteNodeAction extends UndoAction
    {
        ...
        DeleteNodeAction(int nTargetNode)
        {
            ...
            for (int iNode = 0; iNode < getNrOfNodes(); iNode++)
            {
                if (m_ParentSets[iNode].contains(nTargetNode))
                {
                    m_deleteArcActions.addElement(new DeleteArcAction(
                            nTargetNode,
                                                                      iNode));
                }
            }
        ...
        public void undo()
        {
            try
            {
                m_Instances.insertAttributeAt(m_att,
                                              m_nTargetNode);
                int nAtts = m_Instances.numAttributes();
                // update parentsets
                ParentSet[] parentSets = new ParentSet[nAtts];
                ...
                for (int iParentSet = 0; iParentSet < nAtts; iParentSet++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            int nParent = parentSets[iParentSet].getParent(
                                    iParent);
                            if (nParent >= m_nTargetNode)
                            {
                                parentSets[iParentSet].SetParent(iParent,
                                                                 nParent + 1);
                            }
                        }
                    }
                ...
                {
                    DeleteArcAction action = (DeleteArcAction) m_deleteArcActions.elementAt(
                            deletedArc);
                    action.undo();
                }
                m_nPositionX.insertElementAt(m_nPosX,
                                             m_nTargetNode);
                m_nPositionY.insertElementAt(m_nPosY,
                                             m_nTargetNode);
                m_nEvidence.insertElementAt(-1,
                                            m_nTargetNode);
                m_fMarginP.insertElementAt(new float[getCardinality(
    ...
    class DeleteSelectionAction extends UndoAction
    {
        ...
        public DeleteSelectionAction(FastVector nodes)
        {
            ...
            for (int iNode = 0; iNode < nodes.size(); iNode++)
            {
                ...
                {
                    if (!nodes.contains(iNode2) && m_ParentSets[iNode2].contains(
                            nTargetNode))
                    {
                        m_deleteArcActions.addElement(
                                new DeleteArcAction(nTargetNode,
                                                    iNode2));
                    }
                }
            }
        ...
        public void undo()
        {
            try
            {
                for (int iNode = 0; iNode < m_nodes.size(); iNode++)
                {
                    ...
                    m_Instances.insertAttributeAt(m_att[iNode],
                                                  nTargetNode);
                }
                int nAtts = m_Instances.numAttributes();
                // update parentsets
                ...
                for (int iNode = 0; iNode < nAtts; iNode++)
                {
                    if (iTargetNode < m_nodes.size() && (Integer) m_nodes.elementAt(
                            iTargetNode) == (Integer) iNode)
                    {
                        SerializedObject so = new SerializedObject(
                                m_CPT[iTargetNode]);
                        distributions[iNode] = (Estimator[]) so.getObject();
                ...
                {
                    int nTargetNode = (Integer) m_nodes.elementAt(iNode);
                    m_nPositionX.insertElementAt(m_nPosX[iNode],
                                                 nTargetNode);
                    m_nPositionY.insertElementAt(m_nPosY[iNode],
                                                 nTargetNode);
                    m_nEvidence.insertElementAt(-1,
                                                nTargetNode);
                    m_fMarginP.insertElementAt(new float[getCardinality(
    ...
    class DeleteArcAction extends UndoAction
    {
                        ...
                        int nChild)
        {
            try
            {
                ...
                {
                    m_nParents[iParent] = getParent(nChild,
                                                    iParent);
                }
                SerializedObject so = new SerializedObject(
                        m_Distributions[nChild]);
        ...
        public void undo()
        {
            try
            {
                ...
                for (int iParent = 0; iParent < m_nParents.length; iParent++)
                {
                    parentSet.addParent(m_nParents[iParent],
                                        m_Instances);
                }
    ...
    class SetDistributionAction extends UndoAction
    {
        ...
        SetDistributionAction(int nTargetNode,
                              float[][] P)
        {
            try
            {
    ...
    class RenameValueAction extends RenameAction
    {
        ...
        RenameValueAction(int nTargetNode,
                          String sOldName,
                          String sNewName)
        {
            super(nTargetNode,
    ...
    class AddValueAction extends UndoAction
    {
        ...
        AddValueAction(int nTargetNode,
                       String sValue)
        {
            m_nTargetNode = nTargetNode;
            m_sValue = sValue;
        ...
        public void redo()
        {
            addNodeValue(m_nTargetNode,
                         m_sValue);
        } // redo

        public String getUndoMsg()
        {
            return "Value " + m_sValue + " removed from node " + getNodeName(
                    m_nTargetNode);
        }

        public String getRedoMsg()
        {
            return "Value " + m_sValue + " added to node " + getNodeName(
                    m_nTargetNode);
        }
    ...
    class DelValueAction extends UndoAction
    {
        ...
        DelValueAction(int nTargetNode,
                       String sValue)
        {
            try
            {
        ...
        public void undo()
        {
            try
            {
                for (int iNode = 0; iNode < m_nodes.size(); iNode++)
                {
                    int nNode = (Integer) m_nodes.elementAt(iNode);
                    setPosition(nNode,
                                (Integer) m_posX.elementAt(iNode),
                                (Integer) m_posY.elementAt(iNode));
                }
            }
            catch (Exception e)
            {
    ...
    class SetPositionAction extends UndoAction
    {
        ...
        SetPositionAction(int nTargetNode,
                          int nX,
                          int nY)
        {
            m_nTargetNode = nTargetNode;
        ...
        public void setUndoPosition(int nX,
                                    int nY)
        {
            m_nX2 = nX;
            m_nY2 = nY;
    ...
    class SetGroupPositionAction extends UndoAction
    {
        ...
        SetGroupPositionAction(FastVector nodes,
                               int dX,
                               int dY)
        {
            m_nodes = new FastVector(nodes.size());
        ...
        public void undo()
        {
            for (int iNode = 0; iNode < m_nodes.size(); iNode++)
            {
                int nNode = (Integer) m_nodes.elementAt(iNode);
                setPosition(nNode,
                            getPositionX(nNode) - m_dX,
                            getPositionY(nNode) - m_dY);
            }
        } // undo
        ...
        public void setUndoPosition(int dX,
                                    int dY)
        {
            m_dX += dX;
            m_dY += dY;
    ...
    class LayoutGraphAction extends UndoAction
    {
        ...
        LayoutGraphAction(FastVector nPosX,
                          FastVector nPosY)
        {
        ...
        public void undo()
        {
            ...
            {
                setPosition(iNode,
                            (Integer) m_nPosX.elementAt(iNode),
                            (Integer) m_nPosY.elementAt(iNode));
            }
        } // undo
        ...
        public void redo()
        {
            ...
            {
                setPosition(iNode,
                            (Integer) m_nPosX2.elementAt(iNode),
                            (Integer) m_nPosY2.elementAt(iNode));
            }
        } // redo
