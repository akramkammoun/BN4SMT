 ...
 * @author akram
 */
//import weka.classifiers.bayes.MyEditableBayesNet;
...
public class MyMarginCalculator implements Serializable, RevisionHandler, Cloneable
{
    ...
    public JunctionTreeNode[] jtNodes;
    ...
    public int getNode(String sNodeName)
    {
        ...
        {
            if (m_root.m_bayesNet.m_Instances.attribute(iNode).name().equals(
                    sNodeName))
            {
                return iNode;
            }
     ...
     */
    public void calcMargins(MyEditableBayesNet bayesNet) throws Exception
    {
        //System.out.println(bayesNet.toString());
        boolean[][] bAdjacencyMatrix = moralize(bayesNet);
        process(bAdjacencyMatrix,
                bayesNet);
    } // calcMargins

    public void calcFullMargins(MyEditableBayesNet bayesNet) throws Exception
    {
        //System.out.println(bayesNet.toString());
    ...
    public void process(boolean[][] bAdjacencyMatrix,
                        MyEditableBayesNet bayesNet) throws Exception
    {
        int[] order = getMaxCardOrder(bAdjacencyMatrix);
        bAdjacencyMatrix = fillIn(order,
                                  bAdjacencyMatrix);
        order = getMaxCardOrder(bAdjacencyMatrix);
        Set[] cliques = getCliques(order,
                                   bAdjacencyMatrix);
        Set[] separators = getSeparators(order,
                                         cliques);
        int[] parentCliques = getCliqueTree(order,
                                            cliques,
                                            separators);
        // report cliques
            ...
            for (int i = 0; i < nNodes; i++)
            {
                ...
                if (cliques[iNode] != null)
                {
                    ...
                    while (nodes.hasNext())
                    {
                        ...
                        System.out.print(iNode2 + " " + bayesNet.getNodeName(
                                iNode2));
                        if (nodes.hasNext())
                        {
                            System.out.print(",");
        ...
        jtNodes = getJunctionTree(cliques,
                                  separators,
                                  parentCliques,
                                  order,
                                  bayesNet);
    ...
    void initialize(JunctionTreeNode[] jtNodes,
                    int[] order,
                    Set[] cliques,
                    Set[] separators,
                    int[] parentCliques)
                                       ...
                                       MyEditableBayesNet bayesNet)
    {
        ...
        for (int i = 0; i < nNodes; i++)
        {
            ...
            if (cliques[iNode] != null)
            {
                jtns[iNode] = new JunctionTreeNode(cliques[iNode],
                                                   bayesNet,
                                                   bDone);
            }
        }
    ...
    public class JunctionTreeSeparator implements Serializable, RevisionHandler, Cloneable
    {

        private static final long serialVersionUID = 6502780192411755343L;
        public int[] m_nNodes;
        public int m_nCardinality;
        public float[] m_fiParent;
        public float[] m_fiChild;
        public JunctionTreeNode m_parentNode;
        public JunctionTreeNode m_childNode;
        public MyEditableBayesNet m_bayesNet;

        JunctionTreeSeparator(Set separator,
                              MyEditableBayesNet bayesNet,
                              JunctionTreeNode childNode,
         ...
         * separator set
         *
         * @param node: one of the neighboring junciont tree nodes of this
         * separator
         */
        public float[] update(JunctionTreeNode node)
        {
            if (node.m_P == null)
            {
                return null;
            }
            float[] fi = new float[m_nCardinality];
            ...
            {
                int iNodeCPT = getCPT(node.m_nNodes,
                                      node.m_nNodes.length,
                                      values,
                                      order,
                                      m_bayesNet);
                int iSepCPT = getCPT(m_nNodes,
                                     m_nNodes.length,
                                     values,
                                     order,
                                     m_bayesNet);
        ...
        @Override
        public JunctionTreeSeparator clone()
        {
            JunctionTreeSeparator my = null;
            try
            {
                my = (JunctionTreeSeparator) super.clone();
            }
            ...
            my.m_parentNode = this.m_parentNode.clone();
            my.m_childNode = this.m_childNode.clone();
    ...
    public final class JunctionTreeNode implements Serializable, RevisionHandler, Cloneable
    {
         ...
         */
        public float[] m_P;
        public float[][] m_MarginalP;
        public JunctionTreeSeparator m_parentSeparator;
        ...
        public void initializeUp()
        {
            ...
            for (Iterator child = m_children.iterator(); child.hasNext();)
            {
                ...
                for (int iPos = 0; iPos < m_nCardinality; iPos++)
                {
                    int iSepCPT = getCPT(separator.m_nNodes,
                                         separator.m_nNodes.length,
                                         values,
                                         order,
                                         m_bayesNet);
                    int iNodeCPT = getCPT(m_nNodes,
                                          m_nNodes.length,
                                          values,
                                          order,
                                          m_bayesNet);
                    ...
                    int i = 0;
                    values[i]++;
                    while (i < m_nNodes.length && values[i] == m_bayesNet.getCardinality(
                            m_nNodes[i]))
                    {
                        values[i] = 0;
                        i++;
            ...
            else
            {
                ...
                for (int iPos = 0; iPos < m_nCardinality; iPos++)
                {
                    int iSepCPT = getCPT(m_parentSeparator.m_nNodes,
                                         m_parentSeparator.m_nNodes.length,
                                         values,
                                         order,
                                         m_bayesNet);
                    int iNodeCPT = getCPT(m_nNodes,
        ...
        void calcMarginalProbabilities()
        {
            ...
            {
                order[m_nNodes[iNode]] = iNode;
                m_MarginalP[iNode] = new float[m_bayesNet.getCardinality(
                        m_nNodes[iNode])];
            }
            for (int iPos = 0; iPos < m_nCardinality; iPos++)
            {
        ...
        void calculatePotentials(MyEditableBayesNet bayesNet,
                                 Set clique,
                                 boolean[] bDone)
        {
            ...
            for (int iNode = 0; iNode < m_nNodes.length; iNode++)
            {
                ...
                {
                    int nParent = bayesNet.getParent(nNode,
                                                     iParent);
            ...
            for (int iPos = 0; iPos < m_nCardinality; iPos++)
            {
                ...
                for (int iNode = 0; iNode < m_nNodes.length; iNode++)
                {
                    if (bIsContained[iNode])
                    {
                        ...
                        int iCPT2 = getCPT(nNodes,
                                           bayesNet.getNrOfParents(nNode),
                                           values,
                                           order,
                                           bayesNet);
                        float f = (float) bayesNet.getDistributions()[nNode][iCPT2].getProbability(
                                values[iNode]);
                        m_fi[iCPT] *= f;
                    }
                }
                ...
                int i = 0;
                values[i]++;
                while (i < m_nNodes.length && values[i] == bayesNet.getCardinality(
                        m_nNodes[i]))
                {
                    values[i] = 0;
                    i++;
...
//                        System.out.println("m_nCardinality: " + m_nCardinality);
            ////////////////////////////////
            // initialize potential function

            calculatePotentials(bayesNet,
                                clique,
                                bDone);
        ...
        public void setEvidence(int nNode,
                                ArrayList<Integer> iValue,
                                boolean setEvidenceZero) throws Exception
        {
            int[] values = new int[m_nNodes.length];
            int[] order = new int[m_bayesNet.getNrOfNodes()];
            ...
            {
                throw new Exception(
                        "setEvidence: Node " + nNode + " not found in this clique");
            }
            for (int iPos = 0; iPos < m_nCardinality; iPos++)
            {
                if (setEvidenceZero == true)
                {
                    if (iValue.contains(values[nNodeIdx]) == false)
                    {
                        int iNodeCPT = getCPT(m_nNodes,
                                              m_nNodes.length,
                                              values,
                ...
                if (setEvidenceZero == false)
                {
                    if (iValue.contains(values[nNodeIdx]) == true)
                    {
                        int iNodeCPT = getCPT(m_nNodes,
                                              m_nNodes.length,
                                              values,
                                              order,
                                              m_bayesNet);
                        m_P[iNodeCPT] = (float) 1 / iValue.size();
                    }
                }
                // update values
                int i = 0;
            ...
            if (source != this)
            {
                ...
                for (int iPos = 0; iPos < m_nCardinality; iPos++)
                {
                    int iNodeCPT = getCPT(m_nNodes,
                                          ...
                                          order,
                                          m_bayesNet);
                    int iChildCPT = getCPT(nChildNodes,
                                           nNumChildNodes,
                                           values,
                                           order,
                                           m_bayesNet);
        ...
        @Override
        public JunctionTreeNode clone()
        {
            JunctionTreeNode my = null;
            try
            {
                my = (JunctionTreeNode) super.clone();
            }
            catch (CloneNotSupportedException e)
            {
                // this should never happen
            ...
            my.m_children = (Vector) this.m_children.clone();

            my.m_parentSeparator = this.m_parentSeparator.clone();

            return my;
        }
    } // class JunctionTreeNode

    int getCPT(int[] nodeSet,
               int nNodes,
               int[] values,
               int[] order,
               MyEditableBayesNet bayesNet)
     ...
     */
    Set[] getCliques(int[] order,
                     boolean[][] bAdjacencyMatrix) throws Exception
    {
        int nNodes = bAdjacencyMatrix.length;
        ...
        for (int iNode = 0; iNode < nNodes; iNode++)
        {
            for (int iNode2 = 0; iNode2 < nNodes; iNode2++)
            {
                if (iNode != iNode2 && cliques[iNode] != null && cliques[iNode2] != null && cliques[iNode].containsAll(
                        cliques[iNode2]))
                {
                    cliques[iNode2] = null;
                }
     ...
     */
    public boolean[][] moralize(MyEditableBayesNet bayesNet)
    {
        int nNodes = bayesNet.getNrOfNodes();
        boolean[][] bAdjacencyMatrix = new boolean[nNodes][nNodes];
        for (int iNode = 0; iNode < nNodes; iNode++)
    ...
    private void moralizeNode(ParentSet parents,
                              int iNode,
     ...
     *
     * @param bAdjacencyMatrix: n by n matrix with adjacencies in graph of n
     * nodes
                            ...
                            boolean setEvidenceZero) throws Exception
    {
        ...
        {
            throw new Exception(
                    "Could not find node " + nNode + " in junction tree");
        }
        jtNodes[iJtNode].setEvidence(nNode,
                                     iValue,
                                     setEvidenceZero);
    } // setEvidence
    ...
    public static void main(String[] args)
    {
        try
        {
            MyBIFReader bayesNet = new MyBIFReader();
            bayesNet.processFile(args[0]);

            MyMarginCalculator dc = new MyMarginCalculator();
...
//        @Override
//        public MyMarginCalculator clone()
//        {
//            MyMarginCalculator my = null;
//
////            //simple type
////            my.m_debug = this.m_debug;
////            my.m_Margins = this.m_Margins;
////
////            //complex type
////            my.m_root = this.m_root;
////            my.jtNodes = this.jtNodes;
//
//            try
//            {
//                my = (MyMarginCalculator) super.clone();
//            }
//            catch (CloneNotSupportedException e)
//            {
//                // this should never happen
...
//            }
//
////            my.jtNodes = new JunctionTreeNode[this.jtNodes.length];
////            for(int i = 0; i < this.jtNodes.length; i++)
////            {
////                my.jtNodes[i] = this.jtNodes[i].clone();
////            }
//
//
//            my.m_root = this.m_root.clone();
//            my.jtNodes = this.jtNodes.clone();
////            my.m_root.m_children = (Vector) this.m_root.m_children.clone();
////            my.m_root.m_parentSeparator = this.m_root.m_parentSeparator.clone();
//
//
//            return my;
//        }
} // class MyMarginCalculator